...
public class Stream_02_Test {
	...
	public void test_mapToDouble_sum() throws Exception {
		...
		//methode sans mapToDouble
		double result = orders.stream().map(p -> p.getPrice()).filter(i -> i>0).reduce((num1, num2) -> num1 + num2).get();
		
		//methode avec mapToDouble
		double result2 = orders.stream().mapToDouble(Order::getPrice).sum();
